...
import { Badge, Button, Meter, ReadinessRing, Tag } from "@/components/ds";
...
const DAYS: Day[] = [
  ...
  },
  {
    ...
    w: "Fri",
    state: "",
    tasks: [{ t: "Graph drills + spaced-repetition review", m: "75 min · coding", done: false }],
  },
  {
    d: "12",
    w: "Sat",
  ...
  },
  {
    ...
    state: "done",
    label: "Yesterday",
    tasks: [{ t: "Rate limiters & queue design exercise", m: "Completed · 88% self-score", done: true }],
  },
];
      ...
      <div className="stack">
          ...
          <div className="spread">
            <div className="row" style={{ gap: "12px" }}>
              <div className="card-icon" style={{ width: 40, height: 40 }}>
                <Icons.Target size={19} />
              </div>
              <div>
                <div className="serif-h" style={{ fontSize: "17px" }}>
                  Stripe on-site — 14-day plan
                </div>
                <div className="muted" style={{ fontSize: "12.5px", marginTop: "2px" }}>
                  Target: Jun 15 · Day 9 of 14 · on track
                </div>
              </div>
            </div>
            <Badge tone="success">
              <Icons.Check size={10} stroke={3} /> On track
            </Badge>
          </div>
          <div className="row" style={{ gap: "12px", marginTop: "16px" }}>
            <Meter value={64} style={{ flex: 1 }} />
            <span className="dim" style={{ fontSize: "12px", fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>
              64%
            </span>
          </div>
        </div>
        ...
        <div className="surface" style={{ padding: "22px" }}>
          <div className="spread" style={{ marginBottom: "18px" }}>
            ...
            <div className="seg">
              <button data-on="true">Timeline</button>
              <button data-on="false" onClick={() => router.push("/schedule")}>
                Calendar
              </button>
            </div>
          </div>
          {DAYS.map((day) => (
            <div className="day-row" key={day.d}>
              <div className="day-tag">
                <div className={"day-node " + day.state}>{day.state === "done" ? <Icons.Check size={13} stroke={3} /> : day.d}</div>
                <div className="day-date">{day.label || day.w}</div>
              </div>
              <div className="day-body">
                {day.tasks.map((t, j) => (
                  <div className="task" key={t.t}>
                    <span className={"task-check" + (t.done ? " done" : "")}>{t.done && <Icons.Check size={10} stroke={3} />}</span>
                    <div style={{ flex: 1 }}>
                      <div className={"task-title" + (t.done ? " done" : "")}>{t.t}</div>
                      <div className="task-meta">{t.m}</div>
                    </div>
                    {!t.done && day.state === "today" && j === 0 && (
                      <Button variant="subtle" size="sm" onClick={() => router.push("/mock")} iconRight={<Icons.Arrow size={12} />}>
                        Start
                      </Button>
                    )}
                  </div>
      ...
      <div className="stack">
        <div className="surface" style={{ padding: "22px", display: "flex", flexDirection: "column", alignItems: "center" }}>
          ...
          <ReadinessRing
            ...
            caption={
              <>
                On pace for <b style={{ color: "var(--text)" }}>88%</b> by Jun 15
              </>
            }
          />
        </div>
        <div className="surface" style={{ padding: "20px 22px" }}>
          ...
          <Button variant="secondary" size="sm" fullWidth iconLeft={<Icons.Sparkle size={13} />}>
            Regenerate plan
          </Button>
        </div>
      </div>
