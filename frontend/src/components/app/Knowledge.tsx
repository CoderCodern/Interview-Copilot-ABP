...
const REVIEW_STATUS: [string, number, string][] = [
  ["Strong", 12, "var(--success)"],
  ["Steady", 9, "var(--accent)"],
  ["Shaky", 5, "var(--warning)"],
  ["Gaps", 3, "var(--danger)"],
];

const DUE_TODAY = ["Rate limiter design — token bucket", "Exactly-once delivery semantics", "LRU cache implementation"];
    ...
    <div className="two-col">
      <div className="stack">
          ...
          {tab === "bank" && (
            <div style={{ paddingTop: "18px" }}>
              <div className="spread" style={{ gap: "12px", marginBottom: "14px" }}>
                <Input icon={<Icons.Search size={14} />} placeholder="Search questions…" value={q} onChange={(e) => setQ(e.target.value)} style={{ flex: 1 }} />
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "7px", marginBottom: "6px" }}>
                {CATS.map((c) => (
                  <button
                    ...
                    onClick={() => setCat(c)}
                    className="pill"
                    style={cat === c ? { background: "var(--accent-soft)", borderColor: "var(--accent-ring)", color: "var(--accent-deep)" } : undefined}
                  >
                    {c}
                  </button>
                ))}
        ...
        {tab === "bank" && (
          <div className="surface" style={{ overflow: "hidden" }}>
            {filtered.map((x) => (
              <div className="kn-row" key={x.q}>
                <div className="kn-mark">
                  <Icons.Book size={14} />
                </div>
                <div>
            ...
            {filtered.length === 0 && (
              <div style={{ padding: "32px", textAlign: "center" }} className="dim">
                No questions match.
              </div>
            )}
          </div>
        ...
        {tab === "notes" && (
          <div className="surface" style={{ overflow: "hidden" }}>
            {NOTES.map((n) => (
              <div className="note" key={n.t} style={{ padding: "16px 20px" }}>
                <div className="note-title">
                  <span className={"note-check" + (n.done ? "" : " open")}>{n.done && <Icons.Check size={9} stroke={3} />}</span>
                  {n.t}
                </div>
        ...
        {tab === "cards" && (
          <div className="surface" style={{ padding: "40px 24px", textAlign: "center" }}>
            <div className="dz-ico" style={{ margin: "0 auto 14px" }}>
              <Icons.Layers size={22} />
            </div>
            <div className="serif-h" style={{ fontSize: "18px", marginBottom: "6px" }}>
              5 cards due for review
            </div>
            <p className="muted" style={{ fontSize: "13px", maxWidth: "34ch", margin: "0 auto 16px", lineHeight: 1.55 }}>
              Spaced-repetition flips your shaky questions back up at the right moment.
            </p>
      ...
      <div className="stack">
        ...
        <div className="surface" style={{ padding: "18px 22px" }}>
          ...
          <Button variant="subtle" size="sm" fullWidth style={{ marginTop: "16px" }} iconLeft={<Icons.Sparkle size={13} />}>
            Quiz me on these
          </Button>
        </div>
      </div>
