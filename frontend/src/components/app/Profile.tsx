...
import { Avatar, Badge, Button, Input, Select, Switch, Tag } from "@/components/ds";
import { Icons } from "@/components/icons";

type NotifKey = "sessions" | "digest" | "nudges";
...
const CONNECTIONS: [string, string, boolean][] = [
  ["LinkedIn", "Imports roles you're targeting", true],
  ["Google Calendar", "Syncs your study blocks", true],
  ["GitHub", "Surfaces relevant projects", false],
];

export function Profile() {
  const [name, setName] = useState("Coder Codern");
  const [email, setEmail] = useState("coder@example.com");
  const [role, setRole] = useState("Senior Backend Engineer");
  const [notif, setNotif] = useState<Record<NotifKey, boolean>>({ sessions: true, digest: true, nudges: false });
      ...
      <div className="stack">
          ...
          <div className="row" style={{ gap: "16px", marginBottom: "20px" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div className="muted" style={{ fontSize: "12.5px", marginTop: "2px" }}>
                Member since 2024 · Pacific Time
              </div>
            </div>
            <Button variant="ghost" size="sm" iconLeft={<Icons.Edit size={13} />}>
              Change photo
            </Button>
          </div>
        ...
        <div className="surface" style={{ padding: "22px" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
            <span className="dim" style={{ fontSize: "12px", alignSelf: "center", marginRight: "2px" }}>
              Preferred companies:
            </span>
            <Tag tone="accent" removable onRemove={() => {}}>
              Stripe
            </Tag>
            <Tag tone="neutral" removable onRemove={() => {}}>
              Plaid
            </Tag>
            <Tag tone="neutral" removable onRemove={() => {}}>
              Adyen
          ...
          <div className="spread" style={{ marginTop: "22px" }}>
            ...
            <Button variant="primary" size="sm" iconRight={<Icons.Check size={13} stroke={3} />}>
              Save changes
            </Button>
          </div>
        </div>
        ...
        <div className="surface" style={{ padding: "20px 22px" }}>
          ...
          {CONNECTIONS.map(([t, d, on]) => (
            <div className="set-row" key={t}>
              ...
              {on ? (
                <Badge tone="success">
                  <Icons.Check size={9} stroke={3} /> Linked
                </Badge>
              ) : (
