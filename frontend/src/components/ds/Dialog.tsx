...
/**
 * Dialog — centered modal on a warm scrim. Raised paper surface, serif
 * title. Controlled via `open`; `onClose` fires on scrim click or Escape.
 */
export function Dialog({ open, onClose, title, description, children, footer = null, width = 460 }: DialogProps) {
  useEffect(() => {
    if (!open) return;
  ...
  return (
    ...
    >
      ...
      >
        {title && (
          <h3
            style={{ fontFamily: "var(--font-serif)", fontSize: "21px", fontWeight: 500, letterSpacing: "-0.01em", color: "var(--text)" }}
          >
            {title}
          </h3>
        )}
        {description && (
          <p style={{ fontFamily: "var(--font-sans)", fontSize: "13.5px", lineHeight: 1.55, color: "var(--text-2)", margin: "8px 0 0" }}>
            {description}
          </p>
        )}
