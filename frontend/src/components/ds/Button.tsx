...
/**
 * Button — the primary action control. Warm and tactile: primary uses
 * the clay gradient with a top inset highlight; it lifts on hover and
...
export function Button({
  ...
  const sizes: Record<ButtonSize, { padding: string; fontSize: string; radius: string; gap: string }> = {
  ...
  const hoverStyle: CSSProperties =
    hover && !disabled
      ...
      : {};
  const activeStyle: CSSProperties = active && !disabled ? { transform: "translateY(1px) scale(0.99)" } : {};
