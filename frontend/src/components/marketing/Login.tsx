...
/** Mouse-scrub video: dragging the cursor horizontally scrubs the clip. */
function useScrubVideo(sensitivity = 0.9) {
  const ref = useRef<HTMLVideoElement>(null);
  const state = useRef<{ prevX: number | null; target: number; seeking: boolean }>({
    prevX: null,
    target: 0,
    seeking: false,
  });
  useEffect(() => {
    ...
    function onMove(e: MouseEvent) {
      ...
      if (state.current.prevX == null) {
        state.current.prevX = e.clientX;
        return;
      }
      const delta = e.clientX - state.current.prevX;
      state.current.prevX = e.clientX;
      let t = state.current.target + (delta / window.innerWidth) * sensitivity * v.duration;
      t = Math.max(0, Math.min(v.duration, t));
      state.current.target = t;
      if (!state.current.seeking) {
    ...
    function onSeeked() {
      if (!v) return;
      if (Math.abs(v.currentTime - state.current.target) > 0.01) v.currentTime = state.current.target;
      else state.current.seeking = false;
    }
...
function GoogleMark() {
  return (
    <svg width="16" height="16" viewBox="0 0 48 48">
      <path fill="#EA4335" d="M24 9.5c3.5 0 6.6 1.2 9 3.6l6.7-6.7C35.6 2.6 30.2 0 24 0 14.6 0 6.4 5.4 2.6 13.2l7.9 6.1C12.3 13.2 17.6 9.5 24 9.5z" />
      <path fill="#4285F4" d="M46.5 24.5c0-1.6-.1-3.1-.4-4.5H24v9h12.7c-.6 3-2.3 5.5-4.8 7.2l7.4 5.7c4.3-4 6.8-9.9 6.8-17.4z" />
      <path fill="#FBBC05" d="M10.5 28.3c-.5-1.5-.8-3.1-.8-4.8s.3-3.3.8-4.8l-7.9-6.1C1 16.1 0 19.9 0 24s1 7.9 2.6 11.4l7.9-6.1z" />
      <path fill="#34A853" d="M24 48c6.2 0 11.5-2 15.3-5.5l-7.4-5.7c-2 1.4-4.7 2.3-7.9 2.3-6.4 0-11.7-3.7-13.6-9.8l-7.9 6.1C6.4 42.6 14.6 48 24 48z" />
    </svg>
  );
}

const SCRUB_VIDEO_SRC =
  ...
  return (
    <div className="auth">
      ...
      <div className="auth-hint">
        <Icons.Sparkle size={12} /> Move your cursor to scrub the scene
      </div>
      ...
      <main className="auth-stage">
        <div className="auth-card">
          ...
          <h1 className="auth-title">
            {isReg ? (
              <>
                Create your <em>prep</em>.
              </>
            ) : (
              <>
                Sign in to your <em>prep</em>.
              </>
            )}
          </h1>
          <p className="auth-sub">
            {isReg
              ...
              : "Your dashboard, plan and mock sessions are right where you left them."}
          </p>
          ...
          <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            ...
            {!isReg && <Checkbox checked={remember} onChange={() => setRemember(!remember)} label="Keep me signed in" />}
            <Button
              type="submit"
              variant="primary"
              fullWidth
              size="lg"
              disabled={loading}
              iconRight={loading ? undefined : <Icons.Arrow size={14} />}
              style={{ marginTop: "4px" }}
            >
