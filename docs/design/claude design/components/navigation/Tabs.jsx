      ...
      {items.map((it) => {
        ...
        return (
          ...
          >
            {it.label}
            {it.count != null && (
              <span style={{ fontSize: "11px", fontWeight: 500, color: active ? "var(--accent)" : "var(--text-3)", background: "var(--accent-softer)", border: "1px solid var(--border)", padding: "0 6px", borderRadius: "var(--radius-pill)" }}>{it.count}</span>
            )}
          </button>
        );
      })}
