/* Task Routing — map each platform task to a model. Click a target to
   reassign from the model menu. Warm, tactile, no real backend. */
const { Button: RButton } = window.InterviewCopilotDesignSystem_d59c8c || {};
...
function AdminRouting() {
  ...
  const [routes, setRoutes] = React.useState(initial);
  const [open, setOpen] = React.useState(null);

  const assign = (rowId, modelKey) => {
    ...
    <div className="grid-12" style={{ alignItems: "start" }}>
      <div className="acard">
        ...
        <div className="route-map">
          {routes.map((r) => {
            const Ic = I[r.ic] || I.Doc;
            const m = ROUTE_MODELS[r.model];
            return (
              <div className="route-row" key={r.id}>
                <div className="route-task">
                  <span className="route-grip"><I.Drag size={16} /></span>
                  <div className="route-task-ic"><Ic size={16} /></div>
                  <div>
                    <div className="route-task-name">{r.task}</div>
                    <div className="route-task-sub">{r.sub}</div>
                  </div>
                </div>
                <div className="route-arrow"><I.Arrow size={17} /></div>
                <div style={{ position: "relative" }}>
                  <div className="route-target" onClick={(e) => { e.stopPropagation(); setOpen(open === r.id ? null : r.id); }}>
                    ...
                    <I.ChevronRight size={14} style={{ transform: "rotate(90deg)", color: "var(--text-3)" }} />
                  </div>
                  {open === r.id && (
                    <div className="route-menu" onClick={(e) => e.stopPropagation()}>
                      {Object.entries(ROUTE_MODELS).map(([key, mm]) => (
                        <div key={key} className={"route-menu-item" + (key === r.model ? " sel" : "")} onClick={() => assign(r.id, key)}>
                          ...
                          {key === r.model && <I.Check size={13} stroke={2.6} style={{ color: "var(--accent)" }} />}
                        </div>
                      ))}
                    </div>
                  )}
        ...
        <div className="acard-foot">
          ...
          <RButton variant="primary" size="sm" iconRight={<I.Check size={13} stroke={2.6} />}>Save routing</RButton>
        </div>
      </div>

      <div className="stack" style={{ gap: 16 }}>
        <div className="acard">
          <div className="acard-head mb-0" style={{ borderBottom: "none", paddingBottom: 0 }}><h3>Available models</h3></div>
          <div className="acard-body">
            ...
            <p style={{ fontSize: 12, color: "var(--text-3)", marginTop: 16, lineHeight: 1.6 }}>
              Routing is evaluated per request. Cheaper models handle high-volume, low-stakes tasks; Sonnet carries reasoning-heavy flows.
            </p>
          </div>
        </div>

        <div className="acard">
          <div className="acard-head"><h3>Fallback policy</h3></div>
...
window.AdminRouting = AdminRouting;
